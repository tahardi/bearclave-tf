run "configuration_validation" {
  ...
  variables {
    instance_name         = "test-tdx-instance"
  ...
  assert {
    condition     = google_compute_instance.bcl_tdx.name == var.instance_name
    error_message = "Instance name must match input variable"
  }

  assert {
    condition     = google_compute_instance.bcl_tdx.machine_type == var.machine_type
    error_message = "Machine type must match input variable"
  }

  assert {
    condition     = google_compute_instance.bcl_tdx.project == var.project_id
    error_message = "Project ID must match input variable"
  }

  assert {
    condition     = google_compute_instance.bcl_tdx.zone == var.zone
...
run "security_settings" {
  ...
  assert {
    condition     = google_compute_instance.bcl_tdx.confidential_instance_config[0].enable_confidential_compute == true
    error_message = "Confidential compute must be enabled"
  }

  assert {
    condition     = google_compute_instance.bcl_tdx.confidential_instance_config[0].confidential_instance_type == "TDX"
    error_message = "Confidential instance type must be TDX"
  }

  assert {
    condition     = google_compute_instance.bcl_tdx.shielded_instance_config[0].enable_secure_boot == true
    error_message = "Secure boot must be enabled"
  }

  assert {
    condition     = google_compute_instance.bcl_tdx.shielded_instance_config[0].enable_vtpm == true
    error_message = "vTPM must be enabled"
  }

  assert {
    condition     = google_compute_instance.bcl_tdx.shielded_instance_config[0].enable_integrity_monitoring == true
    error_message = "Integrity monitoring must be enabled"
  }

  assert {
    condition     = google_compute_instance.bcl_tdx.boot_disk[0].initialize_params[0].size >= 8
...
run "labeling_strategy" {
  ...
  assert {
    condition     = google_compute_instance.bcl_tdx.labels["tee-type"] == "tdx"
    error_message = "Instance must have tee-type label set to tdx"
  }

  assert {
    condition     = google_compute_instance.bcl_tdx.labels["environment"] == "dev"
    error_message = "Instance must inherit environment label"
  }

  assert {
    condition     = google_compute_instance.bcl_tdx.labels["owner"] == "platform-team"
...
run "service_account_configuration" {
  ...
  assert {
    condition     = google_compute_instance.bcl_tdx.service_account[0].email == var.service_account_email
